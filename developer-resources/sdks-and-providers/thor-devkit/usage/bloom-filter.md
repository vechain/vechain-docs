...
const k = Bloom.estimateK(100)
console.log(k)
// 14

// Create a bloom filter with 14 bits
const bloomFilter = new Bloom(k)

// Add number from 0 to 99 to the bloom filter
for (let i = 0; i < 100; i++) {
    bloomFilter.add(Buffer.from(i + '', 'utf8'))
}

// Positive case (number from 0 to 99 must be present in the bloom filter)
for (let i = 0; i < 100; i++) {
    bloomFilter.test(Buffer.from(i + '', 'utf8')) // All true
}

// Negative case (number from 100 must not be present in the bloom filter)
bloomFilter.test(Buffer.from('100', 'utf8')) // False
```
